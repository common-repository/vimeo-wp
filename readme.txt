=== Plugin Name ===
Contributors: Vimeo WP
Donate link: http://wp-themes.ws
Tags: vimeo, youtube, music, video, widget, widgets
Requires at least: 2.0.2
Tested up to: 3.2.1
Stable tag: 1.0.3

This Vimeo plugin allows you to show your recently uploaded Vimeo videos and display them in a list (With links) as a widget on your blog.

== Description ==

This Vimeo plugin allows you to show your recently uploaded Vimeo videos and display them in a list (With links) as a widget on your blog.

You just need to insert your Vimeo username and the number of videos you want in the list, and everything will be managed for you. You just need to enable the widget.

Installing it is also very simple. See the installation menu for instructions.

== Installation ==

1. Upload `vimeo-wp.php` to the `/wp-content/plugins/` directory
2. Activate the plugin through the 'Plugins' menu in WordPress
3. Edit the Vimeo WP Settings in your Administration Panel
4. Enable the Widget through the Appearance --> Widgets tab.

== Frequently Asked Questions ==

= Does this work? =

Yes.

== Changelog ==

= 1.0.3 =
* Links to Vimeo are now nofollow
* Altered the admin options

= 1.0.2 =
* Uses stripslashes to make inputs more secure

= 1.0.1 =
* Vimeo descriptions shown
* Cleaner HTML code

= 1.0 =
* First Release
